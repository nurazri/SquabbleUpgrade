...
var _prev_rigidbody_mode: int = RigidBody2D.MODE_CHARACTER
...
func select(is_picked: bool = true, is_auto: bool = true) -> void:
	mode = RigidBody2D.FREEZE_MODE_STATIC if is_picked else _prev_rigidbody_mode
...
func _on_Tap_pressed() -> void:
	if get_tree().root.get_node("Game/Pool/BoardMe")._picked_letters.size() >= 7 and $Tap.button_pressed == true:
		$Tap.button_pressed = false
		return
...
func _on_TweenMove_tween_completed(_object, key) -> void:
