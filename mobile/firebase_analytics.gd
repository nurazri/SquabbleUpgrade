...
func _ready():
	if Globals.analytics_turned_on:
		if Engine.has_singleton("FirebaseAnalytics"):
			...
			_fba = ClassDB.instantiate('FirebaseAnalytics')
			print('[FirebaseAnalytics] plugin inited!')
...
func userProperties(props):
	...
	if _fba != null:
		for prop in props:
			_fba.setUserProperty(prop, var_to_str(props[prop]))
...
func revenue(price, _quantity, _product, currency='USD', _signature='', _receipt=''):
	if _fba != null:
