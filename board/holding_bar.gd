...
func remove_letter(this_letter) -> void:
	var direction = [-1,1] 
	var get_direction = direction[randi() % direction.size()]
	print(_letters_array.size())
	await get_tree().create_timer(0.05).timeout
	var get_letter = WordList.spawned_letters[_letters_array[this_letter].id]
	if get_letter.owned_by == Globals.LetterOwnership.POOL:
		_letters_array[this_letter].apply_impulse(get_direction * Vector2.RIGHT * 6000, Vector2.ZERO)
		_letters_array[this_letter].apply_impulse(Vector2.UP * 3000, Vector2.ZERO)
		_letters_array[this_letter].get_node("Col").disabled = false
		_letters_array[this_letter]._in_holding_bar = false
	else:
		_letters_array[this_letter].move_to(Vector2(get_letter.global_position_x, get_letter.global_position_y), 0, get_letter.owned_by, get_letter.cell_index, true, true, false)
		_letters_array[this_letter].get_node("Tap").button_pressed = false
	
	_letters_array.remove(this_letter)
	rearrange_letter()
...
func reset_letter() -> void:
	for l in _letters_array:
		...
		if get_letter.owned_by == Globals.LetterOwnership.POOL:
			...
			if l._initial_global_position_x != get_letter["global_position_x"] && l._initial_global_position_y != get_letter["global_position_y"]:
...
func highlight_snatch(current_tiles, can_snatch: bool = false) -> void:
