...
@export var who_am_i: Globals.LetterOwnership
...
func reset(mode: int, type: int, scorelimit: int, continue_from_tutorial: bool = false) -> void:
	...
	if who_am_i == Globals.LetterOwnership.BOARD_OPPONENT:
		...
		$UI/ScorePanel.set("custom_styles/panel", _StylePanelRed)
...
func set_interactivity(can_interact: bool, timer: float = 0.0) -> void:
	...
	if timer != 0.0:
		...
		tween.tween_property($Background/Freeze_Effect, "modulate", Color(1,1,1,1), 0.15).set_trans(Tween.TRANS_LINEAR)
		
		# Wait for timer
		await get_tree().create_timer(timer).timeout
		
		var tween2 = create_tween()
		tween2.tween_property($Background/Freeze_Effect, "modulate", Color(1,1,1,0), 0.5).set_trans(Tween.TRANS_LINEAR)
...
func protect_board(timer: float = 0.0, booster_texture: Texture2D = null) -> void:
	...
	_boost_protection_current += randi_range(1, 100)
	_current_boost_timer_queue = _boost_protection_current
	$Background/EffectLayer/Timer_Progress.value = 100
	$Background/EffectLayer/Protect_Booster.rect_size = Vector2(0, 775)
	$Background/EffectLayer/Protect_Booster.rect_position = Vector2(476, 19)
	...
	# Fade-in tweens
	var tween = create_tween()
	tween.tween_property($Background/EffectLayer/Protect_Effect, "modulate", Color(1, 1, 1, 1), 0.75).set_trans(Tween.TRANS_LINEAR)
	tween.tween_property($Background/EffectLayer/Timer_Progress, "modulate", Color(1, 1, 1, 1), 0.5).set_trans(Tween.TRANS_LINEAR)
	tween.tween_property($Background/EffectLayer/Timer_Progress, "value", 0, timer).set_trans(Tween.TRANS_LINEAR)
	tween.tween_property($Background/EffectLayer/Protect_Booster, "modulate", Color(7, 7, 7, 1), 0.45).set_trans(Tween.TRANS_LINEAR)
	tween.tween_property($Background/EffectLayer/Protect_Booster, "modulate", Color(0.5, 0.8, 10, 0.45), 0.25).set_trans(Tween.TRANS_LINEAR).set_delay(0.45)
	tween.tween_property($Background/EffectLayer/Protect_Booster, "rect_size", Vector2(765, 775), 0.45).set_trans(Tween.TRANS_LINEAR)
	tween.tween_property($Background/EffectLayer/Protect_Booster, "rect_size", Vector2(535, 775), 0.25).set_trans(Tween.TRANS_LINEAR).set_delay(0.45)
	tween.tween_property($Background/EffectLayer/Protect_Booster, "rect_position", Vector2(94, 19), 0.45).set_trans(Tween.TRANS_LINEAR)
	tween.tween_property($Background/EffectLayer/Protect_Booster, "rect_position", Vector2(209, 19), 0.25).set_trans(Tween.TRANS_LINEAR).set_delay(0.45)
	...
	# Wait for timer using await
	await get_tree().create_timer(timer).timeout

	# Fade-out tweens
	var tween_out = create_tween()
	tween_out.tween_property($Background/EffectLayer/Protect_Effect, "modulate", Color(1, 1, 1, 0), 0.5).set_trans(Tween.TRANS_LINEAR)
	tween_out.tween_property($Background/EffectLayer/Timer_Progress, "modulate", Color(1, 1, 1, 0), 0.5).set_trans(Tween.TRANS_LINEAR)
	tween_out.tween_property($Background/EffectLayer/Protect_Booster, "modulate", Color(0.5, 0.8, 10, 0), 0.45).set_trans(Tween.TRANS_LINEAR)

	# Reset protection flag after timer
		...
		for letter in letters:
			for p in _picked_letters:
				if letter and p and letter.id == p.id:
					...
					if f >= 0:
						_picked_letters.remove_at(f)
	
	_spelled_word = ""
	if who_am_i == Globals.LetterOwnership.BOARD_ME and in_picked_letters:
...
func pick_letter(id: int, is_picked: bool = true) -> void:
	...
	if is_picked:
		...
		if found > -1:
			_picked_letters.remove_at(found)
			if who_am_i == Globals.LetterOwnership.BOARD_ME:
	...
	for l in _picked_letters:
		if l and WordList.spawned_letters.has(l.id):
			...
			var owner = WordList.spawned_letters[l.id]["owned_by"]
			var points = WordList.spawned_letters[l.id]["points"]
			
			if owner == Globals.LetterOwnership.POOL:
				exist_pool_letter = true
				points_total += points
			elif owner == Globals.LetterOwnership.BOARD_OPPONENT:
				points_total += points
			elif owner == Globals.LetterOwnership.BOARD_ME:
				owned_letters += 1
	
	if WordList.is_valid_word(word) and exist_pool_letter:
...
func set_avatar(avatar: Texture, background: Texture) -> void:
...
func set_opponent_name(name: StringName) -> void:
	$UI/OpponentName.text = str(name)
