...
func child(path: String) -> StorageReference:
	...
	var child_path = full_path.path_join(path)
	return storage.ref(child_path)
...
func put_file(file_path: String, metadata: Dictionary = {}) -> StorageTask:
	...
	if not file:
		...
		var failed_task := StorageTask.new()
		failed_task.error = ERR_FILE_CANT_OPEN
		failed_task.finished = true
		return failed_task
	
	var data := file.get_buffer(file.get_length())
	file.close()  # FileAccess closes automatically on free, but explicit is fine
	
	# Override Content-Type based on file extension if not already set
	if not metadata.has("Content-Type"):
		var ext := file_path.get_extension().to_lower()
		metadata["Content-Type"] = MIME_TYPES.get(ext, DEFAULT_MIME_TYPE)
