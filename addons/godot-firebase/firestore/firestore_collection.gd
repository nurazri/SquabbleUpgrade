...
func fetch(document_id : String) -> FirestoreTask:
	if not auth:
...
func delete(document_id : String) -> FirestoreTask:
	...
	task.connect("task_finished", Callable(self, "_on_task_finished").bind(null, document_id), CONNECT_DEFERRED)
	_process_request(task, document_id, url)
	return task
...
func _process_request(task : FirestoreTask, document_id : String, url : String, fields := "") -> void:
	task.connect("error", Callable(self, "_on_error"))
	task._url = url
	task._fields = fields
